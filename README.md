...
- `render` (Render) - Render-функция вида `({ html, context, core, state }) => html`...``
...
- **data** - путь(и) к данным (для динамического контента)
- **expr** - унифицированное выражение с индексами (для сложных случаев)
- **value** - статическое значение (для статического контента)
- **attr** - атрибуты элемента (для элементов)
- **child** - дочерние узлы (для элементов, map, условий)
