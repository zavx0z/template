...
В модуле `data.ts` используется константа `ARGUMENTS_PREFIX` для формирования индексационных плейсхолдеров внутри выражений `expr`.

- Формат плейсхолдера: `${${ARGUMENTS_PREFIX}[i]}`
- По умолчанию может быть пустой строкой или значением `"arguments"` — в зависимости от требований среды выполнения.
- Все места генерации выражений используют эту константу; достаточно изменить её в одном месте.

Примеры:

- При `ARGUMENTS_PREFIX = "arguments"`: `"${arguments[0]} && ${arguments[1]}"`
- При `ARGUMENTS_PREFIX = ""`: `"${[0]} && ${[1]}"` (под конкретный рантайм)

## Правила HTML-форматирования текста и выражений

- Все пробельные символы в выражениях схлопываются до одного пробела, края триммируются.
- Для статического текста применяется схлопывание только для многострочных фрагментов или при избыточных пробелах; однострочный текст без избыточных пробелов сохраняется как есть.
- Это гарантирует согласованность с HTML-стандартом по обработке whitespace.
