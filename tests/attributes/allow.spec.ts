...
describe.each([["iframe","<iframe"],["component","<x-iframe"]])("allow для %s", (_, tag) => {
  ...
  describe("динамические значения", () => {
    it("одно", () => {
      ...
      expect(attrs).toEqual({
        array: { allow: { splitter: ";", values: [{ type: "dynamic", value: "core.permission" }] } },
      })
    })
  })
