...
describe("комплексные тесты с множественными атрибутами", () => {
  describe("элемент area с shape и coords", () => {
    it("простые статические значения", () => {
      const attrs = parseAttributes('shape="rect" coords="0,0,100,100" alt="Прямоугольник" disabled readonly')
    ...
    it("динамические значения в coords", () => {
      const attrs = parseAttributes(
        'shape="circle" coords="${center.x},${center.y},${radius}" alt="Круг" disabled=${context.disabled} ${context.loading && "loading"}'
      )
      expect(attrs).toEqual({
        string: {
    ...
    it("смешанные значения в coords", () => {
      const attrs = parseAttributes(
        'shape="poly" coords="0,0,${width},${height},50,50" alt="Многоугольник" checked=${isChecked} ${context.visible && "visible"} ${context.active && "active"}'
      )
      expect(attrs).toEqual({
        string: {
    ...
    it("сложные условные выражения в coords", () => {
      const attrs = parseAttributes(
        'shape="rect" coords="${isLarge ? "0,0,300,300" : "0,0,100,100"}" alt="Условный прямоугольник" disabled=${context.status === "loading"} ${context.isLarge && "large"} ${context.isActive && "active"}'
      )
      expect(attrs).toEqual({
        string: {
          shape: { type: "static", value: "rect" },
  ...
  describe("элемент img с множественными атрибутами", () => {
    it("src, alt, class, srcset", () => {
      const attrs = parseAttributes(
        'src="image.jpg" alt="Изображение" class="photo main" srcset="image.jpg 1x, image@2x.jpg 2x" loading="lazy" ${context.visible && "visible"} ${context.loaded && "loaded"}'
      )
      expect(attrs).toEqual({
        string: {
    ...
    it("динамические значения в нескольких атрибутах", () => {
      const attrs = parseAttributes(
        'src="${image.url}" alt="${image.alt}" class="${image.classes}" srcset="${image.srcset}" disabled=${context.disabled} ${context.loading && "loading"} ${context.error && "error"}'
      )
      expect(attrs).toEqual({
        string: {
    ...
    it("смешанные значения в разных атрибутах", () => {
      const attrs = parseAttributes(
        'src="base.jpg" alt="Фото ${user.name}" class="avatar ${user.status}" srcset="base.jpg 1x, ${user.highResImage} 2x" readonly={user.readonly} ${user.isAdmin && "admin"} ${user.isVerified && "verified"}'
      )
      expect(attrs).toEqual({
        string: {
  ...
  describe("элемент iframe с allow и sandbox", () => {
    it("простые статические значения", () => {
      const attrs = parseAttributes(
        'src="page.html" allow="camera;microphone" sandbox="allow-scripts allow-same-origin" loading="lazy" ${context.trusted && "trusted"} ${context.secure && "secure"}'
      )
      expect(attrs).toEqual({
        string: {
    ...
    it("динамические значения в allow", () => {
      const attrs = parseAttributes(
        'src="${page.url}" allow="${permissions.join(";")}" sandbox="allow-scripts" disabled=${context.disabled} ${context.loading && "loading"} ${context.error && "error"}'
      )
      expect(attrs).toEqual({
        string: {
  ...
  describe("элемент input с accept и class", () => {
    it("простые статические значения", () => {
      const attrs = parseAttributes(
        'type="file" accept="image/png,image/jpeg" class="file-input required" required disabled ${context.valid && "valid"} ${context.invalid && "invalid"}'
      )
      expect(attrs).toEqual({
        string: {
    ...
    it("динамические значения в accept", () => {
      const attrs = parseAttributes(
        'type="file" accept="${allowedTypes.join(",")}" class="file-input ${isRequired ? "required" : ""}" checked=${isChecked} ${context.valid && "valid"} ${context.invalid && "invalid"}'
      )
      expect(attrs).toEqual({
        string: {
          type: { type: "static", value: "file" },
  ...
  describe("элемент link с rel и class", () => {
    it("простые статические значения", () => {
      const attrs = parseAttributes(
        'rel="stylesheet preload" href="style.css" class="main-styles" disabled ${context.loaded && "loaded"} ${context.cached && "cached"}'
      )
      expect(attrs).toEqual({
        string: {
    ...
    it("динамические значения в rel", () => {
      const attrs = parseAttributes(
        'rel="${isPreload ? "preload" : "stylesheet"}" href="${style.url}" class="styles" disabled=${context.disabled} ${context.loading && "loading"} ${context.error && "error"}'
      )
      expect(attrs).toEqual({
        string: {
  ...
  describe("элемент a с ping и rel", () => {
    it("простые статические значения", () => {
      const attrs = parseAttributes(
        'href="https://example.com" ping="/analytics" rel="noopener noreferrer" disabled ${context.external && "external"} ${context.secure && "secure"}'
      )
      expect(attrs).toEqual({
        string: {
    ...
    it("динамические значения в ping", () => {
      const attrs = parseAttributes(
        'href="${link.url}" ping="${analytics.urls.join(" ")}" rel="noopener" disabled=${context.disabled} ${context.external && "external"} ${context.secure && "secure"}'
      )
      expect(attrs).toEqual({
        string: {
  ...
  describe("элемент meta с accept-charset", () => {
    it("простые статические значения", () => {
      const attrs = parseAttributes(
        'charset="UTF-8" accept-charset="UTF-8 ISO-8859-1" disabled ${context.valid && "valid"} ${context.invalid && "invalid"}'
      )
      expect(attrs).toEqual({
        string: {
  ...
  describe("элемент th с headers и class", () => {
    it("простые статические значения", () => {
      const attrs = parseAttributes(
        'headers="col1 col2" class="header-cell sortable" disabled ${context.sortable && "sortable"} ${context.active && "active"}'
      )
      expect(attrs).toEqual({
        array: {
          headers: [
  ...
  describe("элемент ol с itemref и class", () => {
    it("простые статические значения", () => {
      const attrs = parseAttributes(
        'itemref="item1 item2" class="numbered-list" disabled ${context.ordered && "ordered"} ${context.active && "active"}'
      )
      expect(attrs).toEqual({
        string: {
  ...
  describe("элемент video с sizes и class", () => {
    it("простые статические значения", () => {
      const attrs = parseAttributes(
        'sizes="(max-width: 600px) 100vw, 50vw" class="responsive-video" disabled ${context.playing && "playing"} ${context.paused && "paused"}'
      )
      expect(attrs).toEqual({
        string: {
  ...
  describe("сложные случаи с множественными условными атрибутами", () => {
    it("элемент с условными классами и координатами", () => {
      const attrs = parseAttributes(
        'shape="rect" coords="${isLarge ? "0,0,300,300" : "0,0,100,100"}" class="${isActive ? "active" : "inactive"} ${isVisible ? "visible" : "hidden"}" disabled=${context.disabled} ${context.isLarge && "large"} ${context.isActive && "active"} ${context.isVisible && "visible"}'
      )
      expect(attrs).toEqual({
        string: {
          shape: { type: "static", value: "rect" },
    ...
    it("элемент с множественными динамическими атрибутами", () => {
      const attrs = parseAttributes(
        'src="${image.url}" alt="${image.alt}" class="${image.classes}" srcset="${image.srcset}" sizes="${image.sizes}" disabled=${context.disabled} ${context.loading && "loading"} ${context.error && "error"} ${context.visible && "visible"}'
      )
      expect(attrs).toEqual({
        string: {
          src: { type: "dynamic", value: "${image.url}" },
    ...
    it("элемент с смешанными статическими и динамическими атрибутами", () => {
      const attrs = parseAttributes(
        'src="https://example.com" allow="${permissions.join(";")}" sandbox="allow-scripts ${user.canAccess ? "allow-same-origin" : ""}" class="embed" disabled=${context.disabled} ${context.trusted && "trusted"} ${context.secure && "secure"} ${context.loading && "loading"}'
      )
      expect(attrs).toEqual({
        string: {
  ...
  describe("граничные случаи", () => {
    it("элемент только с строковыми атрибутами", () => {
      const attrs = parseAttributes(
        'id="main" title="Заголовок" data-test="value" disabled ${context.visible && "visible"} ${context.active && "active"}'
      )
      expect(attrs).toEqual({
        string: {
    ...
    it("элемент только с массивными атрибутами", () => {
      const attrs = parseAttributes(
        'class="container" rel="nofollow noopener" ping="/analytics" disabled ${context.visible && "visible"} ${context.active && "active"}'
      )
      expect(attrs).toEqual({
        string: {
    ...
    it("элемент с пустыми значениями", () => {
      const attrs = parseAttributes(
        'class="" rel="" title="" disabled ${context.visible && "visible"} ${context.active && "active"}'
      )
      expect(attrs).toEqual({
        boolean: {
          disabled: { type: "static", value: true },
