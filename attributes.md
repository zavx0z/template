...
#### Поддерживаемые стандартные булевые атрибуты:

- `disabled`, `readonly`, `required`, `checked`, `selected`
- `multiple`, `autoplay`, `loop`, `muted`, `controls`
- `autofocus`, `novalidate`, `formnovalidate`, `open`
- `reversed`, `scoped`, `seamless`, `async`, `defer`
- `download`, `hidden`, `ismap`, `noshade`, `nowrap`, `compact`
